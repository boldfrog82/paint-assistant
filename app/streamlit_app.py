...
def get_catalogs() -> tuple[List[dict], dict[str, dict[str, Decimal]]]:
    """Load catalogue data once per session."""
...
def _product_options(query: str) -> dict[str, dict]:
    """Return a mapping of product name to the product record for the UI."""
...
search_query = st.text_input(
    "Search products", placeholder="Type to search the catalogue"
)
product_options = _product_options(search_query)

selected_product: str | None = None
if product_options:
    selected_product = st.selectbox(
        "Product", list(product_options.keys()), key="product_select"
    )
else:
...
if selected_product:
    ...
    if packs:
        ...
        if st.button("Add to Quote", type="primary"):
            try:
                item = quote_row(
                    selected_product, pack_choice, quantity_value, discount_value
                )
